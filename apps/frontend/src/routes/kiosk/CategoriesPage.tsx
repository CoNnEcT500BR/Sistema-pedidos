    ...
    <div className="flex min-h-screen flex-col bg-gray-50">
      ...
      <header className="sticky top-0 z-20 flex items-center justify-between border-b border-gray-200 bg-white px-4 py-3 shadow-sm">
        ...
        >
          ...
          <span className="font-semibold">{t('Início')}</span>
        </button>

        <h1 className="text-xl font-black text-gray-900">{t('CARDÁPIO')}</h1>
      ...
      <main className="flex flex-1 flex-col gap-4 p-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={t('Buscar categoria...')}
        ...
        {loading && (
          <div className="flex flex-1 items-center justify-center">
            <div className="flex flex-col items-center gap-3 text-gray-400">
              ...
              <span>{t('Carregando cardápio...')}</span>
            </div>
          </div>
        )}
        ...
        {error && !loading && (
          <div className="flex flex-1 items-center justify-center">
            <div className="rounded-2xl bg-red-50 p-6 text-center">
              <p className="text-lg font-semibold text-red-600">{t('Ops! Não foi possível carregar o cardápio.')}</p>
              <p className="mt-1 text-sm text-red-400">{t('Verifique a conexão e tente novamente.')}</p>
              <button
                onClick={refetch}
        ...
        {!loading && !error && (
          <>
            {filtered.length === 0 ? (
              <p className="text-center text-gray-400 mt-8">{t('Nenhuma categoria encontrada.')}</p>
            ) : (
