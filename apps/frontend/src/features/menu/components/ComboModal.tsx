    ...
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent className="max-h-[90vh] overflow-y-auto p-0 sm:max-w-lg">
        <DialogHeader className="sticky top-0 z-10 border-b border-gray-100 bg-white px-6 py-4">
          <DialogTitle className="text-xl font-bold text-gray-900">{combo.name}</DialogTitle>
          <DialogDescription className="sr-only">
            Modal de personalizacao do combo {combo.name}. Selecione upgrades e ingredientes antes de adicionar ao carrinho.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-5 px-6 pb-6 pt-4">
          ...
          {burgerItem && (
            <div className="flex flex-col gap-2">
              <h4 className="text-sm font-semibold uppercase tracking-wide text-gray-600">
                Personalizar lanche do combo ({burgerItem.name})
              </h4>
              {burgerAddonsLoading ? (
                <span className="text-sm text-gray-400">Carregando ingredientes do lanche...</span>
              ) : burgerAddons.length === 0 ? (
                <span className="text-sm text-gray-400">Esse lanche nao possui personalizacao.</span>
              ) : (
                <IngredientsEditor
                  addons={burgerAddons}
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            <p className="font-semibold">Regras deste combo</p>
            <ul className="mt-2 list-disc space-y-1 pl-5">
              <li>O combo personalizado e adicionado 1 unidade por vez.</li>
              <li>Upgrades de mesmo grupo sao exclusivos (ex.: bebida M ou G).</li>
              <li>Em sabores de bebida, apenas 1 sabor pode ficar selecionado.</li>
            </ul>
          </div>
          ...
          {drinkItem && (
            <div className="flex flex-col gap-2">
              <h4 className="text-sm font-semibold uppercase tracking-wide text-gray-600">
                Personalizar bebida do combo ({drinkItem.name})
              </h4>
              {drinkAddonsLoading ? (
                <span className="text-sm text-gray-400">Carregando opcoes da bebida...</span>
              ) : drinkAddons.length === 0 ? (
                <span className="text-sm text-gray-400">Essa bebida nao possui personalizacao.</span>
              ) : (
                <IngredientsEditor
                  addons={drinkAddons}
          ...
          <div className="flex flex-col gap-2">
            ...
            {allAddonsLoading ? (
              <span className="text-sm text-gray-400">Carregando opcoes...</span>
            ) : upgradeOptions.length === 0 ? (
              <span className="text-sm text-gray-400">Sem upgrades para este combo.</span>
            ) : (
              <>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4">
            <h4 className="text-sm font-semibold uppercase tracking-wide text-gray-600">Voce escolheu</h4>

            {!hasAnyCustomization ? (
              <p className="mt-2 text-sm text-gray-500">Sem personalizacoes adicionais neste combo.</p>
            ) : (
              <div className="mt-3 flex flex-col gap-3 text-sm text-gray-700">
                {selectedUpgradeSummary.length > 0 && (
                  <div className="rounded-xl border border-blue-100 bg-blue-50 p-3">
                    <p className="text-[10px] font-semibold uppercase tracking-wide text-blue-600">Upgrades</p>
          ...
          >
            <span className="text-lg font-bold">Adicionar Combo</span>
